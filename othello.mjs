...
import {isValidMove, hasValidMove} from './validator.mjs';
import {collectCaptures} from './captureUtil.mjs';
/**
  ...
  async play() {
    ...
    while (true) {
      ...
      try {
        ...
        // TODO: implement
        const valid = isValidMove(board, turn, move);


        if (valid) {
          board.positions[move.row][move.col] = turn;

          collectCaptures(board, turn, move ).forEach((capture) => {
            board.positions[capture.row][capture.col] = turn;
          });

          

          turn = turn === Color.BLACK? Color.WHITE: Color.BLACK;
          // if (hasValidMove(board, nextTurn)){
          //   turn = nextTurn;  
